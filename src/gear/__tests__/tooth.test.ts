...
describe('tooth generation', () => {
  ...
  it('buildToothProfile returns two segments per flank and mirrored flank', () => {
    ...
    expect(tooth.mirroredFlank[0].p0.x).toBeCloseTo(tooth.flank[1].p0.x, 8)
    expect(tooth.mirroredFlank[0].p0.y).toBeCloseTo(-tooth.flank[1].p0.y, 8)
    expect(tooth.mirroredFlank[1].p3.x).toBeCloseTo(tooth.flank[0].p3.x, 8)
    expect(tooth.mirroredFlank[1].p3.y).toBeCloseTo(-tooth.flank[0].p3.y, 8)
  })
})
